...
import { CONFIRM_EMAIL_WITHOUT_TOKEN, GET_AUTH_TOKEN,RESET_PASSWORD_WITHOUT_TOKEN } from 'src/api/endpoints';
...
export class Authentication{
    ...
    constructor(apiClient: ApiClient){
     ...
     async getAuthToken(username: string, password: string, systemId?: string): Promise<string> {
        const data = {
            username,
            password,
            client_id: '1STPARTY.APP',
            grant_type: 'password',
            scope: 'resourceApi offline_access profile fileServiceApi notificationApi',
        };

        const base = process.env.API_BASE_URL ?? process.env.BASE_URL;
        //const apiVersion = process.env.API_VERSION ?? 'v1';
        if (!base) throw new Error('API base URL not configured. Set API_BASE_URL or BASE_URL.');
     ...
     */
    async resetPasswordWithoutToken(credentials: { username: string; password: string }, apiKey?: string, systemId?: string): Promise<object> {
        const apiVersion = process.env.API_VERSION ?? 'v1';
        const baseUrl = this.apiClient.baseURL.replace(/\/+$/,'') + `/${apiVersion}`;
       ...
       async confirmEmailWithoutToken(username: string, apiKey?: string, systemId?: string): Promise<object> {
        const apiVersion = process.env.API_VERSION ?? 'v1';
        const baseUrl = this.apiClient.baseURL.replace(/\/+$/,'') + `/${apiVersion}`;
