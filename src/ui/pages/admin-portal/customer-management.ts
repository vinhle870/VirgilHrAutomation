...
export class CustomerManagementPage extends BasePage {
  ...
  public async upgradePlan(customer: CustomerInfo, planToUpgrade: string) {
    if (customer.company.companySize?.includes("500+"))
      throw new Error(
        "The current plan is maximun so it is impossible to upgrade",
      );
    ...
    const detailButtonLocator = rawPhoneNumber.replace(
      "phoneNumberValue",
      phoneNumber,
    );
    ...
    //click upgrade plan
    await (
      await this.getLocator(CustomerDetailLocator.customerDetailButton)
    ).click();
    ...
    if (!(await planToUpgradeEl.isVisible()))
      throw new Error("The upgraded plan does not exist");
    ...
    if (customer.bankStranferToUpgradePlan) {
      const bankStranferButtonEl = this.page.locator(
        UpgradePlanLocator.bankStranfer,
      );

      await bankStranferButtonEl.click();

      await (
        await this.getLocator(UpgradePlanLocator.upgradeNowButton)
      ).click();
    } else
      await (
        await this.getLocator(UpgradePlanLocator.requestPaymentButton)
      ).click();
  }
}
