...
export class LoginPage {
    ...
    async loginWithValidAccount(url:string, username:string, password:string) {
        ...
        const userField = await LocatorHandling.getLocator(this.page, LoginPageLocators.txt_UserName);
        await userField.fill(username);
        const passField = await LocatorHandling.getLocator(this.page, LoginPageLocators.txt_Password);
        await passField.fill(password);
        const signInBtn = await LocatorHandling.getLocator(this.page, LoginPageLocators.btn_SignIn);
