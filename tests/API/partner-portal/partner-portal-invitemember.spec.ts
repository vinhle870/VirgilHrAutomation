...
test.describe(
  ...
  test("TC64: API- POST /Partner/Manage/Partner/Business with added members: Return 200-OK and correct Response",
    {
      tag: [
        "@TC64",
        "@API",
  ...
  let customerWithMember = await test.step("Pre-condition: Generate member data for invite payload", async () => {
    ...
    const { partnerPlansList, planItem } = await test.step(
      "Pre-condition: Get partner plans list and resolve plan item",
      async () => {
  ...
  await test.step("Verified business list is correct", async () => {
    ...
    await test.step(
      "Verified CALL API -> get team members list reflects invited members",
      async () => {
  ...
  test("TC65: API- POST Partner/Manage/Partner/Business/Invite: Return 200-OK and correct Response",
    {
      tag: [
        "@TC65",
        "@API",
    ...
    const preConditionData = await test.step(
      "Pre-condition: Prepare department, products, and master plan",
      async () => {
    ...
    const partnerInfo = await test.step(
      "Pre-condition: Create partner info using PartnerBuilder",
      async () => {
        return await DataFactory.partnerBuilder()
          .withIsPublic(true)
          .withWhoPay(0)
    ...
    const customerWithMember = await test.step(
      "Pre-condition: Generate member data for invite payload",
      async () => {
        return await new CustomerBuilder()
          .forMemberPortal()
          .withMember({ role: 3 }) // User role
    ...
    const partnerToken = await test.step(
      "Steps: Activate partner account and get auth token",
      async () => {
        delay(30000);
    ...
    const { planItem } = await test.step(
      "Pre-condition: Get partner plans list and resolve plan item",
      async () => {
        const partnerPlansList =
          await partnerPortalService.getPartnerPlansList(partnerToken);
  ...
  test("TC67: API- POST /Partner/Manage/Partner/Business with WhoPay=1 (Customer): 1st invited Member = Owner",
    {
      tag: [
        "@TC67",
        "@API",
    ...
    const partnerInfo = await test.step(
      "Pre-condition: Create partner info using PartnerBuilder (WhoPay=Customer)",
      async () => {
        return await DataFactory.partnerBuilder()
          .withIsPublic(true)
    ...
    if (partnerResponse) {
      ...
      await test.step(
        "Verified team members list (first invited member is owner)",
        async () => {
          const teamMembersList = await partnerPortalService.getTeamMembersList(
            businessList.entities[0].id,
            partnerToken,
