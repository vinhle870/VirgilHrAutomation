...
import { virgilHRPlans, biginHRPlans } from "src/constant/department.plan.qa";
...
test.describe("Admin Portal -> Customer Management",
  ...
  test.beforeAll(async ({ apiClient, adminPortalService }) => {
    await test.step("Pre-condition: Call API -> create partner",
      async () => {
        ...
        const productTypesAndNamesToSend: ProductInfo[] = await testData.getProductTypesBasedDepartmentId(
                                                                departmentID);
  ...
  test("TC017_API Verify that new customer can be Added under PartnerID return 201-Created and correct Response", 
    {
    ...
    await test.step(
      "SEND POST API /Manage/Consumers: Create a new consumer to create Customer",
      async () => {
        if (!consumerData) {
  ...
  test("TC018_API Verify Customer creation under a HR System (Partner) will return 201-Created and correct Response",
    {
      tag: [
        "@TC018",
        "@API",
    ...
    await test.step(
      "Pre-condition: Resolve partner, product type, and build consumer payload",
      async () => {
        const partnerName = partnerData.partnerInfo?.name;
        if (!partnerName) {
          throw new Error("PARTNER_NAME is not configured");
    ...
    await test.step(
      "SEND POST API /Manage/Consumers: Create customer under HR System (Partner)",
      async () => {
        if (!consumerData) {
          throw new Error("consumerData was not created in pre-condition step");
        }
  ...
  test("TC020_API Verify Customer creation with Trial Subscription will return 201-Created and correct Response",
    {
      tag: [
        "@TC020",
        "@API",
    ...
    await test.step(
      "Pre-condition: Resolve partner, product type, and build trial subscription payload",
      async () => {
    ...
    await test.step(
      "SEND POST API /Manage/Consumers: Create customer with trial subscription",
      async () => {
        if (!consumerData) {
          throw new Error("consumerData was not created in pre-condition step");
        }
  ...
  test("TC022_API Verify Customer creation with Bank Transfer = ON will return 201-Created and correct Response",
    {
      tag: [
        "@TC022",
        "@API",
    ...
    await test.step(
      "Pre-condition: Resolve partner, product type, build bank transfer ON payload",
      async () => {
        const partnerName = process.env.PARTNER_NAME;
        if (!partnerName) {
          throw new Error("PARTNER_NAME is not configured");
    ...
    await test.step(
      "SEND POST API /Manage/Consumers: Create customer with bank transfer ON",
      async () => {
        if (!consumerData) {
          throw new Error("consumerData was not created in pre-condition step");
        }
  ...
  test("TC023_API Verify Customer creation with Bank Transfer = OFF will return 201-Created and correct Respons",
    {
      tag: [
        "@TC023",
        "@API",
    ...
    await test.step(
      "Pre-condition: Resolve partner and build consumer payload (bank transfer OFF)",
      async () => {
        const partnerName = process.env.PARTNER_NAME;
        if (!partnerName) {
          throw new Error("PARTNER_NAME is not configured");
    ...
    await test.step(
      "SEND POST API /Manage/Consumers: Create customer with bank transfer OFF",
      async () => {
        if (!consumerData) {
          throw new Error("consumerData was not created in pre-condition step");
        }
    ...
    await test.step(
      "GET consumer by ID: Verify subscription is null (bank transfer OFF)",
      async () => {
        consumerById = await adminService.getConsumerById((resp as any).id);
  ...
  test("TC030_API Verify For Free Trial accounts, the user is also assigned a plan along with a limited number of free usage days.",
    {
      tag: [
        "@TC030",
        "@API",
    ...
    await test.step(
      "Pre-condition: Resolve partner, product type, build free trial payload",
      async () => {
        const partnerName = process.env.PARTNER_NAME;
        if (!partnerName) {
          throw new Error("PARTNER_NAME is not configured");
    ...
    await test.step(
      "SEND POST API /Manage/Consumers: Create free trial customer",
      async () => {
        if (!consumerData) {
          throw new Error("consumerData was not created in pre-condition step");
        }
    ...
    await test.step(
      "GET consumer by ID: Verify plan name and trial duration",
      async () => {
        consumerById = await adminService.getConsumerById((resp as any).id);
