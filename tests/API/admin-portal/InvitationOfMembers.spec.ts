    ...
    test(
      ...
      async ({ apiClient, authenticationService, adminPortalService, partnerPortalService, authFlow, memberPortalService }) => {
        ...
        await test.step("Pre-condition: CALL API /Partner/Manage/Partner/Business -> Add business with partner token", async () => {
          ...
          await authenticationService.confirmEmailWithoutToken(partnerEmail, undefined, "5");

          partnerToken = await authenticationService.getAuthToken(partnerEmail, tempPassword, "5");

          const business = await partnerPortalService.createBusiness(partner, "Team", masterPlanId, undefined, customerWithMember.members, partnerToken);
      ...
      async ({ apiClient, authenticationService, adminPortalService, partnerPortalService, authFlow, memberPortalService }) => {
        ...
        await test.step("For each team: create business, invite member, accept invitation, verify GET Payment/subscription/me", async () => {
          for (let i = 0; i < 2; i++) {
            ...
            // Create business
            await partnerPortalService.createBusiness(partner, businessName, masterPlanId, undefined, customerWithMember.members, partnerToken);
            ...
            const paymentSubscriptionResp = await memberPortalService.getPaymentSubscription(invitedMember);
