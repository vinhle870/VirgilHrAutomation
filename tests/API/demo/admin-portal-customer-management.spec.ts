...
test.describe('Admin Portal - Customer Management', () => {
    ...
    test('TC017_API Verify that new customer can be Added under PartnerID return 201-Created and correct Response', async ({ apiClient, authenticationService }, testInfo) => {
   ...
   test('TC018_API Verify Customer creation under a HR System (Partner) will return 201-Created and correct Response', async ({ apiClient, authenticationService }, testInfo) => {
    const base = process.env.API_BASE_URL ?? process.env.BASE_URL;
    const username = process.env.API_USERNAME ?? process.env.ADMIN_USERNAME;
    const password = process.env.API_PASSWORD ?? process.env.ADMIN_PASSWORD;
    testInfo.skip(!base, 'API_BASE_URL is not configured');
    ...
    test('TC020_API Verify Customer creation with Trial Subscription will return 201-Created and correct Response', async ({ apiClient, authenticationService }, testInfo) => {
    const base = process.env.API_BASE_URL ?? process.env.BASE_URL;
    const username = process.env.API_USERNAME ?? process.env.ADMIN_USERNAME;
    const password = process.env.API_PASSWORD ?? process.env.ADMIN_PASSWORD;
    testInfo.skip(!base, 'API_BASE_URL is not configured');
  ...
  test('TC022_API Verify Customer creation with Bank Transfer = ON will return 201-Created and correct Response', async ({ apiClient, authenticationService }, testInfo) => {
   const base = process.env.API_BASE_URL ?? process.env.BASE_URL;
    const username = process.env.API_USERNAME ?? process.env.ADMIN_USERNAME;
    const password = process.env.API_PASSWORD ?? process.env.ADMIN_PASSWORD;
    testInfo.skip(!base, 'API_BASE_URL is not configured');
    ...
    test('TC023_API Verify Customer creation with Bank Transfer = OFF will return 201-Created and correct Respons', async ({ apiClient, authenticationService }, testInfo) => {
    const base = process.env.API_BASE_URL ?? process.env.BASE_URL;
    const username = process.env.API_USERNAME ?? process.env.ADMIN_USERNAME;
    const password = process.env.API_PASSWORD ?? process.env.ADMIN_PASSWORD;
    testInfo.skip(!base, 'API_BASE_URL is not configured');
  ...
  test('TC030_API Verify For Free Trial accounts, the user is also assigned a plan along with a limited number of free usage days.', async ({ apiClient, authenticationService }, testInfo) => {
    const base = process.env.API_BASE_URL ?? process.env.BASE_URL;
    const username = process.env.API_USERNAME ?? process.env.ADMIN_USERNAME;
    const password = process.env.API_PASSWORD ?? process.env.ADMIN_PASSWORD;
    testInfo.skip(!base, 'API_BASE_URL is not configured');
