...
test.describe('MemberPortalService - signUpConsumer', () => {
  test('creates a new consumer successfully', async ({ apiClient, memberPortalService, authenticationService, planPage }, testInfo) => {
    ...
    const consumerData = await DataFactory.generateCustomerInfo(partnerInfo.partnerId, partnerInfo.departmentId);
    //*****---------------------------------------------------*****
    ...
    const resetResp = await authenticationService.resetPasswordWithoutToken({ username: (consumerData.getAccountInfo() as any).email, password: tempPassword }, undefined, "4");

    //Activate the user account if needed (depends on system settings)
    await authenticationService.confirmEmailWithoutToken((consumerData.getAccountInfo() as any).email, undefined, "4");
    ...
    const consumerToken = await authenticationService.getAuthToken((consumerData.getAccountInfo()  as any).email, tempPassword, "4");
    ...
    await planPage.buyPlan(planUrl, (consumerData.getAccountInfo() as any).email, tempPassword,validCardInfo);
    ...
    const newConsumerToken = await authenticationService.getAuthToken((consumerData.getAccountInfo()  as any).email, tempPassword, "4");
