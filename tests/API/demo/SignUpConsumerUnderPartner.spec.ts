...
test.describe('MemberPortalService - signUpConsumer', () => {
  test('creates a new consumer successfully', async ({ apiClient, memberPortalService, authenticationService, planPage }, testInfo) => {
    ...
    await authenticationService.confirmEmailWithoutToken((consumerData as any).email, undefined, "4");
    ...
    await planPage.buyPlan(planUrl, (consumerData as any).email, tempPassword,validCardInfo);
    ...
    const newConsumerToken = await authenticationService.getAuthToken((consumerData as any).email, tempPassword, "4");
